...
IF NOT EXISTS(
    ...
    WHERE t.name = 'TB_ODS_INVESTIMENTO_ANALITICO'
        AND s.name = 'ODS'
)
BEGIN
    CREATE TABLE ODS.TB_ODS_INVESTIMENTO_ANALITICO(
        NR_ID_INVESTIMENTO INT NOT NULL PRIMARY KEY,
...
# ======================================
query_stage = "SELECT * FROM STG.TB_STAGE_INVESTIMENTO_ANALITICO"
df_stage = pd.read_sql(query_stage, engine)

if df_stage.empty:
    print("⚠️ Nenhum dado encontrado na Stage (STG.TB_STAGE_INVESTIMENTO_ANALITICO). Nada a carregar.")
else:
    print(f"✅ {len(df_stage)} linhas encontradas na Stage.")

    # Renomear colunas (Stage → ODS)
    rename_map = {
        "ID_INVESTIMENTO": "NR_ID_INVESTIMENTO",
    ...
    # Carga para a ODS
    with engine.begin() as conn:
        conn.execute(sa.text("TRUNCATE TABLE ODS.TB_ODS_INVESTIMENTO_ANALITICO"))
        df_stage.to_sql("TB_ODS_INVESTIMENTO_ANALITICO", conn, schema="ODS", if_exists="append", index=False)
        print(f"🚀 {len(df_stage)} registros carregados em ODS.TB_ODS_INVESTIMENTO_ANALITICO")

        # Limpar Stage
        conn.execute(sa.text("TRUNCATE TABLE STG.TB_STAGE_INVESTIMENTO_ANALITICO"))
        print("🧹 Stage STG.TB_STAGE_INVESTIMENTO_ANALITICO truncada com sucesso.")